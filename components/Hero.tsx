...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // Guard: avoid double-init (React StrictMode)
    ...
    try {
      ...
      const renderer = new THREE.WebGLRenderer({
        canvas,
        alpha: true,
        antialias: !(/Mobi|Android/i.test(navigator.userAgent)), // disable AA on mobile
      ...
      const pMat = new THREE.PointsMaterial({ size: 0.055, color: 0x4ade80, transparent: true, opacity: 0.6 });
      scene.add(new THREE.Points(pGeo, pMat));

      // Wireframe shapes
      const shapes: { mesh: THREE.Mesh; rx: number; ry: number }[] = [];
      const geos: THREE.BufferGeometry[] = isMobile
      ...
      // Central torus ring (skip on mobile)
      let bigTorusGeo: THREE.BufferGeometry | null = null;
      let bigTorusMat: THREE.Material | null = null;
      let bigTorus: THREE.Mesh | null = null;
      if (!isMobile) {
        bigTorusGeo = new THREE.TorusGeometry(3, 0.018, 6, 100);
        bigTorusMat = new THREE.MeshBasicMaterial({ color: 0x22c55e, transparent: true, opacity: 0.05 });
        mats.push(bigTorusMat);
        bigTorus = new THREE.Mesh(bigTorusGeo, bigTorusMat);
      ...
      const onMove = (e: MouseEvent) => {
        mx2 = e.clientX / window.innerWidth - 0.5;
        my2 = e.clientY / window.innerHeight - 0.5;
      };
      ...
      return () => {
        ...
        pGeo.dispose();
        if (bigTorusGeo) bigTorusGeo.dispose();
      ...
      console.warn("Hero WebGL init failed:", err);
    }
  }, []);
  ...
  return (
    <section id="beranda" className="relative min-h-screen overflow-hidden flex items-center">
      ...
      {/* Deco blobs */}
      <div
        className="absolute -top-20 -left-20 w-72 sm:w-96 h-72 sm:h-96 rounded-full bg-green-500/20 blur-[80px] pointer-events-none"
        style={{ animation: "morphBlob 10s ease-in-out infinite" }}
      />
      <div
        className="absolute bottom-0 right-0 w-64 sm:w-80 h-64 sm:h-80 rounded-full bg-emerald-700/15 blur-[80px] pointer-events-none"
        style={{ animation: "morphBlob 14s 3s ease-in-out infinite" }}
      />

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full pt-12 lg:pt-16">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center min-h-[85vh]">
          ...
          <div style={{ animation: "fadeUp 0.9s cubic-bezier(0.16,1,0.3,1) both" }}>
            {/* Badge */}
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold uppercase tracking-widest border border-green-400/30 bg-green-400/10 text-green-400 mb-5 sm:mb-6">
            ...
            {/* Heading */}
            <h1
              className="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-black leading-[0.9] mb-5 sm:mb-6 text-white"
            ...
            <div className="flex flex-wrap gap-3 sm:gap-4">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2.5 bg-green-500 hover:bg-green-400 text-white font-semibold px-5 sm:px-7 py-3.5 sm:py-4 rounded-2xl transition-all hover:scale-105 hover:shadow-2xl hover:shadow-green-500/40 text-sm sm:text-base"
              >
              ...
              <a
                ...
                className="flex items-center gap-2 text-white/80 hover:text-white border border-white/20 hover:border-white/40 px-5 sm:px-7 py-3.5 sm:py-4 rounded-2xl transition-all font-medium text-sm sm:text-base"
              >
                Lihat Produk
            ...
            <div className="flex flex-wrap gap-4 sm:gap-6 mt-8 sm:mt-10">
              {["100% Organik", "PIRT Terdaftar", "Gratis Ongkir Malang"].map((item) => (
                <div key={item} className="flex items-center gap-2 text-white/50 text-xs sm:text-sm">
                  <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
          ...
          >
            <div className="relative w-full max-w-md ml-auto">
              <div
                className="w-full h-[480px] xl:h-[520px] rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl shadow-black/50 relative"
      ...
      <div className="absolute bottom-9 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-50 text-white pointer-events-none">
        <div className="w-px h-10 sm:h-12 bg-gradient-to-b from-transparent to-current" style={{ animation: "fadeIn 1s 0.5s both" }} />
