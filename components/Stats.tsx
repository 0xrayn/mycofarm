...
function StatCard({ target, suffix, label, icon }: typeof STATS[0]) {
  ...
  return (
    <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-4 sm:p-6 text-center hover:-translate-y-1 hover:bg-white/[0.07] transition-all duration-300">
      <div className="text-2xl sm:text-4xl mb-2">{icon}</div>
      <div
        className="font-black leading-none mb-1.5 text-base-content"
        style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(1.6rem, 5vw, 3.5rem)" }}
      >
        <span ref={numRef}>0{suffix}</span>
      </div>
      <div className="text-xs sm:text-sm opacity-50">{label}</div>
    </div>
  );
}
...
export default function Stats() {
  return (
    <section className="py-12 sm:py-20 bg-base-200">
